...
/*
 instanceof 연산자
 	...
 	형변환(즉 상속관게가 있으면)이 가능하면 true, 
 	불가능하면 false를 반환한다.
 	- 객체간에 형변환이 되려면 반드시 두 클래스간에 상속관게가 있어야한다.
...
class GoldPaperBox extends PaperBox {
	public void goldWrap() {
		System.out.println("GoldPaperWrap로 포장합니다.");
	}
}
...
public class E09InstanceOf {
	...
	/*
	 -여기서 instanceof 연산산자를 통해 형변환 가능여부를 판단하는
	 이유는 전달된 매개변수가 최상위 클래스인 Box타입으로 받고있기 때문이다.
	 -Box타입으로 인자를 받게되면 자동으로 업캐스팅(자동형변환)이
	 되기 때문에 자식클래스의 멤버메소드를 호출 할 수 없게된다.
	 -이때 자식의 멤버메소드를 호출하기 위해서는 다운캐스팅(명시적형변환)
	 이 필요하다.
	 -따라서 전달된 인자를 각각의 타입으로 형변환이 가능한지 판단한 후
	 다운캐스팅을 진행,멤버메소드를 호출할 수 있게된다.
	 */
