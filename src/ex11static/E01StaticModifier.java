...
-정적변수는 JVM(자바가상머신)에 의해 프로그램이 로드되는
	시점에 초기화 된다.
-main메소드가 실행되기전에 이미 초기화 되므로 main 메소드
...
접근방법
	...
	-클래스 외부
		...
		*변수로 접근할꺼면 굳이 static으로 쓸 이유가 없으니까.
 */

class MyStatic {
	/*
	[멤버변수] 인스턴스형 멤버변수는 반드시 클래스의 객체를
	생성한 후 참조변수를 통해 접근해야한다.
	 */
	int instanceVar;
	/*
	...
	/*
	[정적 멤버벰소드] 정적메소드 안에서는 인스턴스 멤버를 사용할 수 없다.
	정적멤버만 사용이 가능하다.
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 MySatic 클래스의 인스턴스형 멤버변수에 접근하는 방법
		 -> 인스턴스 생성 후 찬조변수를 통해 접근한다.
		 */
		...
		/*
		 방법2 : 인스턴스 변수처럼 참조변수를 통해 접근 할 수 있다.
		 	단, 이경우는 정적변수를 인스턴스 변수로 착각 할 수
		 	있으므로 권장하지 않는다.
		 */
		...
		System.out.println("ms2.instanceVar=" + ms1.instanceVar);
		...
		System.out.println("ms1.saticVar=" + ms1.staticVar);
		System.out.println("ms1.saticVar=" + ms2.staticVar);
		...
		//MyStatic.instanceMethod();//에러발생(차조변수를 통해서만 호출가능)
		MyStatic.staticMethod();
	}
