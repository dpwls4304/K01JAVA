...
/*
 ...
 -반환값이 없다. 즉 특정 연산후 값을 반환할수 없다.
 	따라서 void를 쓸 수 없다.
 -그 외는 메소드가 가진 모든 특성을 가지고 있다. (매개변수전달,
 	오버로딩 등)
 	
 생성자 내에서 this의 사용법
 용도 1 : 동일 클래스내에서 다른 생성자를 호출할 때 사용한다.
 	단, 생성자 내에서만 호출 가능하다.
 	사용법 :
 		this(인자1,인자2, ....);
 
 용도2 : 생성자내에서 멤버변수와 매개변수의 이름이 동일할 떄
 	구분하기 위해 사용한다.
 	this.변수 => 멤버변수
 	변수 => 생성자의 매개변수
...
public class PersonConstructor {
	...
	public PersonConstructor(String name, int age) {
		/*
		 this() 는 생성자내에서 다른 생성자를 호출할 때 사용한다.
		 단, 생성자내에서만 사용가능하고, 멤버메소드에서는
		 사용이 불가능하다.
		 */
		this(name, age, "미상");
